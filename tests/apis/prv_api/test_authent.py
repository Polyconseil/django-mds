...
def test_create_delete_application(client):
    ...
    response = client.post(
        url, data=data, content_type="application/json", **auth_header(SCOPE_PROVIDER)
    )
...
def test_long_lived_token(mocked, client):
    ...
    response = client.post(
        reverse("mds_prv_api:long_lived_token"), **auth_header(SCOPE_PROVIDER)
    )
